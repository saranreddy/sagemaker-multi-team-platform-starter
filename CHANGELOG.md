...
### Fixed (Live Test v0.1.0)
- Added `*.tfvars`, `*.auto.tfvars`, `crash.log`, and `.terraform/` to `.gitignore` (prevents accidental commit of real ARNs/emails in template forks)
- Added multi-platform hashes to `.terraform.lock.hcl` (darwin_arm64, darwin_amd64, linux_amd64, linux_arm64) to prevent lock file modifications on macOS arm64
- Enhanced smoke test isolation check to capture and print AccessDenied proof, and fail on network errors instead of counting them as isolation
